...
void PrintStats(vector<Person> persons) {
	auto female_end = partition(persons.begin(), persons.end(), [&](const Person& p){return p.gender == Gender::FEMALE;});
	auto female_employed_end = partition(persons.begin(), female_end, [&](const Person& p){return p.gender == Gender::FEMALE && p.is_employed;});
	auto male_employed_end = partition(female_end, persons.end(), [&](const Person& p){return p.gender == Gender::MALE && p.is_employed;});
